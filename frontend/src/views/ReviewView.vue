        ...
        <div class="p-4 max-h-96 overflow-y-auto">
          ...
          >
            <div 
              ...
              class="font-medium text-sm text-gray-900 mb-2 cursor-pointer hover:text-blue-600 transition-colors"
            >
              {{ article.title }}
            </div>
        ...
        <div class="space-y-2">
          ...
          >
            <div class="flex-1">
              <div 
                v-if="op.article_id"
                @click="openArticleById(op.article_id)"
              ...
              <div class="flex items-center gap-2 mt-1 flex-wrap">
                ...
                <span v-if="op.source" class="text-xs text-gray-500">• {{ op.source }}</span>
                <span v-if="op.score !== null && op.score !== undefined" class="text-xs text-blue-600 font-medium">评分: {{ op.score }}</span>
                <a
                  v-if="op.link"
                  :href="op.link"
                  target="_blank"
...
const openArticleModal = async (article) => {
  // 如果文章没有 content，尝试从 API 获取完整数据
  if (!article.content && article.id) {
    try {
      const response = await axios.get(`${API_BASE}/articles/${article.id}`)
      modalArticle.value = response.data.article
    } catch (error) {
...
const openArticleById = async (articleId) => {
  try {
